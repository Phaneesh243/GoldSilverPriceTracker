...
import { defaultWeight, inr, validWeight, weightedReference, weightLabel, weightsFor } from "../../lib/metals-presentation";
export default function MetalQuoteCards({ initial, showTools = false, uniformWeight = false }: { initial: MetalPrice[]; showTools?: boolean; uniformWeight?: boolean }) {
  const metal = initial[0]?.key || "gold";
  const [weight, setWeight] = useState(() => defaultWeight(metal));
  useEffect(() => { if (!showTools) return; try { const saved = JSON.parse(localStorage.getItem(`gsp-metal-weight-${metal}`) || "null"); if (validWeight(metal, saved)) setWeight(saved); } catch { /* Storage is optional. */ } }, [metal, showTools]);
  ...
  return <section aria-label="Metal reference prices" className="metals-quotes"><div className="metals-section-heading"><div><h2>Today’s metal references</h2><p className="metals-note">Provider references × daily FX · INR · five-minute cache · not retail quotes</p></div><button className="metals-refresh" disabled={busy} onClick={() => void refresh(true)}><RefreshCw size={16} aria-hidden="true" className={busy ? "metals-spinning" : undefined} />{busy ? "Checking…" : "Refresh prices"}</button></div><span className="metals-sr-only" role="status">{busy ? "Checking reference prices" : "Price check complete"}</span>{error ? <p role="status" className="metals-error">{error}</p> : null}
    <p className="metals-basis">India · INR · IST — International reference converted to INR — not a local retail quotation.</p>
    {showTools ? <div className="metals-price-controls"><label>Market<select aria-label="Market"><option value="IN">India · INR</option></select></label><label>Display weight<select aria-label="Display weight" value={weight} onChange={event => { const next = Number(event.target.value); if (!validWeight(metal, next)) return; setWeight(next); try { localStorage.setItem(`gsp-metal-weight-${metal}`, JSON.stringify(next)); } catch { /* Calculation remains usable. */ } }}>{weightsFor(metal).map(value => <option value={value} key={value}>{weightLabel(metal, value)}</option>)}</select></label><span className="metals-note">Reference basis, not a purity-specific retail offer. Other countries need verified local sources.</span></div> : null}
    <div className="metals-card-grid" aria-busy={busy}>{displayed.map(q => <article className="metals-quote-card" key={q.key} data-metal={q.key} data-availability={q.freshness}>
      <div className="metals-quote-heading"><span className="metals-symbol" aria-hidden="true">{q.symbol}</span><h3><Link className={!showTools ? "metals-card-link" : undefined} href={q.route}>{q.name}</Link></h3><span className="metals-status">{q.freshness === "fresh" ? "Fresh" : q.freshness === "stale" ? "Stale" : "No feed"}</span></div>
      <QuoteValue quote={q} weight={uniformWeight ? 1 : showTools ? weight : defaultWeight(q.key)} />
    ...
    {quotes.some(q => q.key === "gold") && quotes.some(q => q.key === "silver") ? <p className="metals-note">Gold–silver ratio: {ratio !== null ? `${ratio.toFixed(2)} (computed per gram, observations within five minutes)` : "Unavailable until matching fresh observations exist."} A lower unit price does not mean better investment value.</p> : null}{showTools && displayed.length === 1 ? <ReferenceTools quote={displayed[0]} /> : null}</section>;
}
function QuoteValue({ quote, weight }: { quote: MetalPrice; weight: number }) {
  const amount = weightedReference(quote.price, weight);
  return <div className="metals-quote-value"><strong className="metals-price">{amount !== null ? inr(amount) : "Unavailable"}</strong><span className="metals-note">Per {weightLabel(quote.key, weight)} · {quote.key === "copper" ? "HG benchmark (lb convention)" : "fine-metal reference"}</span>{weight !== 1 && quote.price !== null ? <span className="metals-note">{inr(quote.price)} {quote.unitLabel}</span> : null}</div>;
}
function ReferenceTools({ quote }: { quote: MetalPrice }) {
  return <><section id="weights"><h2>Weight breakdown</h2><p className="metals-note">Same refreshed reference and observation as the card above; not a retailer offer. Gold purity-specific dealer rates are not supplied; a karat conversion is theoretical, not an assay.</p><div className="metals-table" tabIndex={0} role="region" aria-label="Weight breakdown"><table><caption>Reference multiplied by weight; no charges or taxes.</caption><thead><tr><th>Quantity</th><th>Reference value (INR)</th></tr></thead><tbody>{weightsFor(quote.key).map(weight => <tr key={weight}><td>{weightLabel(quote.key, weight)}</td><td>{quote.price !== null ? inr(quote.price * weight) : "Unavailable"}</td></tr>)}</tbody></table></div></section><MetalCalculator metal={quote.key} price={quote.freshness === "fresh" ? quote.price : null} /></>;
}
